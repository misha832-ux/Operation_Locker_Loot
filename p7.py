...
def spawn_traps():
    ...
    for _ in range(TRAP_COUNT):
        ...
        trap_tiles.append({
            'i':i,'j':j,
            'vanish':False,
            'timer':random.uniform(5,10) # start with random delay
        })
...
def draw_floor():
    ...
    for i in range(ROWS):
        for j in range(COLS):
            is_trap = any(t['i']==i and t['j']==j for t in trap_tiles)
            color = None
            if is_trap:
                trap = next(t for t in trap_tiles if t['i']==i and t['j']==j)
                if trap['vanish']:
...
def draw_bank_walls():
    ...
    # 🔹 Extra detail: draw black grid lines on walls (bank style)
...
def draw_vault_door():
    ...
    glColor3f(0.7, 0.7, 0.7)  # steel gray
    glutSolidCube(1.0)
    glPopMatrix()
    ...
    glColor3f(0.85, 0.85, 0.85)  # brighter steel
...
def check_collisions():
    ...
    for t in treasures:
        if not t['collected']:
            if math.hypot(player_pos[0]-t['pos'][0], player_pos[1]-t['pos'][1]) < 20:
                t['collected']=True
                ...
                if score >= LOCKER_SCORE and not locker_visible:
                    ...
                    locker_pos[1] = 0
    if locker_visible:
      if math.hypot(player_pos[0]-locker_pos[0],player_pos[1]-locker_pos[1])<25:
        global mission_complete
        mission_complete = True
        game_over = True


    if laser_on and not laser_forced_off:
        for L in lasers:
...
def idle():
    ...
    global player_falling, player_fall_z, lives, game_over
    global game_time, time_up
    global camera_angle_y, camera_angle_x

    now=time.time()
    dt=(now-last_time) if last_time else 1/60.0
    last_time=now
    ...
    for w in walls:
        w['y'] += w['dir']*WALL_SPEED*dt
        if abs(w['y'])>WALL_LIMIT:
            w['dir']*=-1
    
    for trap in trap_tiles:
        if trap['i'] == 0:
            trap['vanish'] = False   # always visible
            trap['timer'] = TRAP_VANISH_TIME
            continue
        if not tile_cheat:   # <-- freeze traps if tile_cheat is True
            trap['timer'] -= dt
            if trap['timer'] <= 0:
                trap['vanish'] = not trap['vanish']
                trap['timer'] = TRAP_VANISH_TIME
